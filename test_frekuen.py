...
location_dict = {
    'jakarta': 'jakarta', 'bandung': 'jawa barat', 'cirebon': 'jawa barat', 'semarang': 'jawa tengah', 'surabaya': 'jawa timur', 'demak': 'jawa tengah', 'padang': 'sumatra barat', 'garut': 'jawabarat',
...
# Daftar kata kunci jenis bencana
banjir_dict = {
    'banjir rob': 'banjir rob', 'banjir bandang': 'banjir bandang', 'banjir sungai': 'banjir sungai', 'banjir lahar': 'banjir lahar', 'gempa bumi': 'gempa bumi', 'tanah longsor': 'tanah longsor',
}

# Fungsi untuk mengekstrak jenis bencana dari teks
...
# Ekstrak jenis bencana untuk setiap klaster
banjir_types = {cluster_id: extract_banjir_type(
    docs, banjir_dict) for cluster_id, docs in clusters.items()}

# Buat nama klaster berdasarkan jenis bencana dan lokasi teratas
cluster_names = {}
for cluster_id in clusters.keys():
    ...
    cluster_name = f"{banjir_type} - {location}"
...
for cluster_id in clusters.keys():
    ...
    print("Top 3 Term:")
    for idx, (term, freq) in enumerate(frequencies[cluster_id].items(), 1):
        print(f"{idx}. {term}: {freq}")
        if idx == 3:  # Hanya menampilkan 3 term teratas
            break
