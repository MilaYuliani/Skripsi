...
# Daftar lokasi yang lebih spesifik (kota, kabupaten, provinsi)
locations = {
    ...
    'sumatra selatan': 'sumatra selatan', 'sumatra utara': 'sumatra utara', 'yogyakarta': 'yogyakarta'
}

# Fungsi untuk mengekstrak provinsi dari teks berdasarkan kota/kabupaten


def extract_locations(text, locations):
    found_locations = []
    for loc, prov in locations.items():
        if re.search(r'\b' + re.escape(loc.lower()) + r'\b', text.lower()):
...
if df_clusters is not None and df_tfidf is not None:
    ...
    df_clusters['locations'] = df_clusters['text'].apply(
        lambda x: extract_locations(x, locations))
    ...
    scatter = plt.scatter(df_clusters['pca_one'], df_clusters['pca_two'], c=df_clusters['cluster_id'].map(
        lambda x: unique_clusters.tolist().index(x)), cmap='viridis')
    ...
    for cluster_id, keywords in keyword_profiles.items():
        ...
        for word, freq in keywords.items():
            print(f'  {word}: {freq}')
        # Determine cluster name based on most common location or keywords
        ...
        if most_common_location:
            ...
            for word, _ in keywords.items():
                if word in locations:
                    cluster_name = "banjir " + locations[word]
                    break
            else:
                cluster_name = "banjir " + \
                    ' '.join([word for word in keywords.index[:3]])
        cluster_names[cluster_id] = cluster_name
        top_topic = keywords.index[0]
        print(f'  Topik Utama: {top_topic}')
